...
def text_to_image(prompt):
    ...
    for attempt in range(max_retries):
        try:
            ...
            headers = {"Authorization": "Bearer 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"}  # Replace with your actual API key
            ...
            return img 
                ...
                # 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
                url = "https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-xl-base-1.0"
                headers = {"Authorization": "Bearer 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"}  # Replace with your actual API key
                response = requests.post(url, headers=headers, json=params)
                response.raise_for_status()  # Raise an exception for HTTP errors
                answers = response.content
                img = Image.open(io.BytesIO(answers))
